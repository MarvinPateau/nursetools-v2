...
const ACTIVE_SERVICE_KEY = 'nurse.activeService';

function readActiveService(): ServiceId {
  try {
    const raw = localStorage.getItem(ACTIVE_SERVICE_KEY) as ServiceId | null;
    if (raw && raw in SERVICE_PRESETS) return raw;
  } catch {
...
export function ServicePresetSettings() {
  const [service, setService] = useState<ServiceId>(() => readActiveService());
  ...
  return (
    <Card title="Protocoles service" subtitle="Définissez le preset actif pour les calculateurs">
      ...
      <div className="mt-3 rounded-xl border border-border bg-surface px-3 py-3">
        ...
        <div className="text-xs text-muted mt-1">
          Concentration standard: {preset.standardConcentrationMgMl} mg/mL · Référence règle de 3: {preset.regleTroisReferenceMgMl} mg/mL
        </div>
...
export function DoseToolView() {
  const [service, setService] = useState<ServiceId>(() => readActiveService());

  useEffect(() => {
    const onFocus = () => setService(readActiveService());
...
export function InfusionToolView() {
  return (
    <section className="mt-0 space-y-6">
      <InfusionRate />
      <DripRate />
...
export function BiologyNormsToolView() {
  return (
    <section className="mt-0 space-y-6">
      <BiologyNorms />
    </section>
